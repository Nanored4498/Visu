...
template<typename T> struct Stack<T&> {
	...
	template<typename G> static void add(const G &x) requires std::is_fundamental_v<T> { return Stack<T>::add(x); }
};
...
int cons2(lua_State *L, const std::string &name, std::integer_sequence<int, inds...>) {
	new(lua_newuserdata(L, sizeof(T))) T(Stack<Args>::get(L, inds + 1)...);
